...
def dashboard():
    ...
    try:
        ...
        user_login = db.user.find_one({"username": payload["id"]})
            # Count total users (level 2 users)
        user_count = db.user.count_documents({'level': 2})

        # Count total messages (Pesan)
        message_count = db.saran.count_documents({})

        # Count total products
        product_count = db.product.count_documents({})

        if(user_login['level'] == 1):
            return render_template('adminDashboard.html', user_login=user_login, user_count=user_count, message_count=message_count, product_count=product_count)
        else:
...
def posting():
    ...
    try:
        ...
        if 'file_give' in request.files:
            file = request.files.get('file_give')  # Corrected the field name here
